...
pub fn keplerToCart(elem: KeplerElements, t: f32, mu: f32) [2]Vec3 {
    ...
    var rt = math.rotationZ(-arg).mul(Mat3, math.rotationX(i - TAU / 4.0).mul(Mat3, math.rotationZ(-long))).dot(ot);
    var rt_v = math.rotationZ(-arg).mul(Mat3, math.rotationX(i - TAU / 4.0).mul(Mat3, math.rotationZ(-long))).dot(ot_v);
