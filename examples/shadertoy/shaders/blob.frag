...
float dist(vec3 r)
{
   vec3 pos = vec3(cos(iTime), sin(iTime), 0);
   pos *= sin(iTime/2);
    return sphere(pos, r, 1.+sin(iTime)*snoise(vec4(r, sin(iTime/100)*100)));
}

vec2 delta = vec2(0., DEL);
...
void main()
{
    ...
    fragColor = vec4(HueShift(ray(pos, dir), iTime), 1.0);
}
